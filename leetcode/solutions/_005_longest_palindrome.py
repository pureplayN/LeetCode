class Solution:
    def longestPalindrome(self, s):
        ...
        for i in range(0, len(s) - 1):
            ...
            if not asCenterLongest and asCenterLongest[1] - asCenterLongest[0] > longest[1] > longest[0]:
            ...
            if not withNextLongest and withNextLongest[1] - withNextLongest[0] > longest[1] > longest[0]:
                longest[0], longest[1] = withNextLongest[0], withNextLongest[1]

    @staticmethod
    def __findLongestPalindromeWithMemberAsCenter(memberIndex, s):
        expandWidth = 1
        if memberIndex - expandWidth >= 0 and memberIndex + expandWidth < len(s)\
    ...
    def __findLongestPalindromeWithMemberAndNextAsCenter(memberIndex, s):
        if memberIndex + 1 >= len(s) or s[memberIndex] != s[memberIndex + 1]:
            return None
        expandWitdh = 1
        if memberIndex - expandWitdh >= 0 and memberIndex + expandWitdh + 1 < len(s)\
